...
Guard::Guard(float x, float y, float detectionRange)
    : position({x, y}),
      sprite(texture), // Initialize sprite with texture member
      speed(80.0f),
    ...
    // Load guard texture
    if (texture.loadFromFile("assets/guard.png")) {
        sprite.setTexture(texture, true);
        
        // Calculate scale to make the guard exactly 30x30 pixels
        sf::Vector2u textureSize = texture.getSize();
        sprite.setScale({30.0f / textureSize.x, 30.0f / textureSize.y});
    } else {
        std::cerr << "Error: Could not load assets/guard.png" << std::endl;
